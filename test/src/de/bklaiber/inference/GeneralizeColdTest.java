...
public class GeneralizeColdTest {
	...
	public void checkGeneralizationPositive() {
		...
		generalizations.addElement("(contact(X,Y) * cold(Y))[0.020752]<X!=Y>");
		...
		classifiedClass1.add(bc);
		classifiedClass1.add(cb);
	...
	generalizations.addElement("(contact(X,Y) * cold(Y))[0.0]<X=Y>");
	generalizations.addElement("(contact(X,Y) * cold(Y))[0.02075229708752361]<X!=Y>");
